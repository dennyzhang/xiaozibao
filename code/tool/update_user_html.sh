...
## --
## Created : <2013-01-31>
## Updated: Time-stamp: <2013-07-15 22:13:59>
##-------------------------------------------------------------------
...
function update_user_html() {
    ...
    # copy resource file
    /bin/cp -r $XZB_HOME/code/tool/smarty_html/templates/resource $user_dir

    # update main page
    python_script="import jinja_html; jinja_html.generate_list_user_post(\"$userid\", \"$date\", \"$index_html\")"

    command="(cd $XZB_HOME/code/tool/smarty_html; python -c '${python_script}')"
    eval $command
    if [ $? -ne 0 ]; then
    ...
    command="(cd $XZB_HOME/code/show_article/tool; python -c '${python_script}')"
    eval $command
    if [ $? -ne 0 ]; then
